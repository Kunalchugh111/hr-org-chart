...
# =========================================================
# 2) Premium Light SaaS Styling (Streamlit UI)
# =========================================================
st.markdown(
    """
    <style>
...
with st.sidebar:
    ...
    st.caption("Tip: On Streamlit Cloud, large org charts need iframe scrolling. This app enables it automatically.")
...
if df is None:
    ...
    # Apply any stored draft moves (server-side only)
    ...
    # Filter by Sub Function if selected
    if selected_sub != "All" and "Sub Function" in base_df.columns:
        base_df = base_df[base_df["Sub Function"] == selected_sub]

    # Valid IDs for manager linkage
    valid_ids = set(base_df["Clean_Emp_Code"].dropna().astype(str).tolist()) if "Clean_Emp_Code" in base_df.columns else set()
    ...
    for _, row in base_df.iterrows():
        ...
        # If manager not present in filtered dataset, treat as root
    ...
    # Make it robust if Designation column missing
    ...
    # =========================================================
    # 6) Render HTML Component (iframe)
    # =========================================================
    html_template = f"""
    <!DOCTYPE html>
    <html lang="en">
      ...
      <style>
        ...
        * {{ box-sizing: border-box; }}
        ...
        .node-card {{
          ...
          width: 240px; /* slightly wider = better readability */
          padding: 0;
          background: var(--card);
          border: 1px solid var(--border);
          border-radius: 14px;
        ...
        /* FIX: Better long-text handling (clamp + tooltip in JS) */
        .emp-name, .emp-title {{
          overflow: hidden;
          text-overflow: ellipsis;
          display: -webkit-box;
    ...
    <body>
      <div class="toolbar">
        ...
        <div class="zoom-controls">
          ...
          <span style="font-size:0.75rem; color:#334155; min-width:40px; text-align:center; font-weight:800" id="zoom-level">100%</span>
          <button class="tool-btn" onclick="zoom(0.1)">+</button>
      ...
      <div class="modal-bg" id="modal-bg" onclick="closeModal()">
        <div class="action-sheet" onclick="event.stopPropagation()">
          ...
          <div class="emp-summary">
            ...
            <div>
              <div style="font-weight:900; color:#0f172a" id="sel-name">Name</div>
              <div style="font-size:0.85rem; color:#334155; font-weight:700" id="sel-title">Title</div>
            </div>
          </div>
      ...
      <script>
        ...
        function createNode(node) {{
          ...
          // Tooltips: title="" will show full text on hover
        ...
        searchInput.addEventListener('input', (e) => {{
          ...
          allEmps
            ...
            .forEach(emp => {{
              ...
              div.innerHTML = `<b>${{escapeHtml(emp.name)}}</b> <span style="color:#334155; font-weight:700">— ${{escapeHtml(emp.title)}}</span>`;
        ...
        // Zoom
        function zoom(delta) {{
          state.zoom = Math.max(0.3, Math.min(1.6, state.zoom + delta));
        ...
        function resetZoom() {{
          ...
          // Optional: scroll to top-left-ish
          const wrapper = document.getElementById('canvas-wrapper');
          wrapper.scrollTop = 0;
          wrapper.scrollLeft = 0;
        }}

        // CSV Download (fixed quoting)
        ...
        function showToast(msg) {{
          ...
          t.style.cssText = 'position:fixed;bottom:20px;right:20px;background:#0f172a;color:#fff;padding:10px 18px;border-radius:10px;font-size:0.9rem;font-weight:800;z-index:999;transform:translateY(20px);opacity:0;transition:0.3s;';
          t.textContent = msg;
    ...
    # =========================================================
    # FIX: Streamlit Cloud iframe scrolling + larger height
    # =========================================================
    # You can raise height to 1600/2000 for very large orgs.
